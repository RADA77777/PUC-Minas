	    ...
	    if(notFim)
   		    ...
   		    while(!feof(arq))
		    {
			    ...
			    if(isSubstring(linha, "infobox vcard"))
			    {
				    ...
				    char nome[100], apelidos[1000], tecnico[100], estadio[100];
...
int* filtrarData(char dataString[])
{
	...
	char numeroChar[1000], auxString[1000];

	strcpy(numeroChar, removerTags(dataString, '<', '>'));
	strcpy(numeroChar, removerTags(numeroChar, '&', ';'));
	strcpy(auxString, dataString);	
	strcpy(dataString, retornarEntreTags(numeroChar, "(", ")"));
	...
	/* Aqui confirmaremos que data[] esta no formato dia/mes/ano. 
	* Para isso, copiamos a <auxString> em <numeroChar>.
	* A variavel <auxString> vai conter todo o conteudo que esta 
	* entre Founded e Ground, tirando elementos HTML.
	* A logica eh ver se nomes de meses sao substrings de <numeroChar>
	* Se for, eh pq o mes que eh substring deve ser o valor numerico da variavel <mes>.
	* Caso o valor numerico de <mes> seja diferente do valor do mes no mundo real, ele
	* eh trocado com a variavel dia (Ex: <mes> = 3 e  subString(linha, Abril) retorna True. Isso
	* nao pode acontecer, pois abril eh o mes 4. Entao, assumimos que 3 eh o dia! <dia> assume o valor
	* de <mes>, e <mes> vira 4) 
	*/
	...
	for(int i = 0; i < 12; i++)
		...
		if(isSubstring(numeroChar, meses[i]) && mes != i+1)
		{
			if(dia == i+1)
			{
				dia = mes;
